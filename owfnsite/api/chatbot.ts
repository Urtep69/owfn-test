...
import { DISTRIBUTION_WALLETS, QUICKNODE_RPC_URL } from '../constants.ts';

async function getLiveBlockchainData() {
    try {
        const connection = new Connection(QUICKNODE_RPC_URL, 'confirmed');
        
        // 1. Get presale total SOL balance
        const presalePublicKey = new PublicKey(DISTRIBUTION_WALLETS.presale);
        const presaleBalance = await connection.getBalance(presalePublicKey);
        const presaleTotalSOL = presaleBalance / LAMPORTS_PER_SOL;

        // 2. Get last donation signature
        const impactPublicKey = new PublicKey(DISTRIBUTION_WALLETS.impactTreasury);
        const signatures = await connection.getSignaturesForAddress(impactPublicKey, { limit: 1 });
        const lastDonationSignature = signatures.length > 0 ? signatures[0].signature : 'None yet';

        return {
            presaleTotal: `${presaleTotalSOL.toFixed(2)} SOL`,
            lastDonationTx: lastDonationSignature,
        };
    } catch (error) {
        console.error("Failed to fetch live blockchain data:", error);
        return {
            presaleTotal: 'data unavailable',
            lastDonationTx: 'data unavailable',
        };
    }
}
...
export default async function handler(req: any, res: any) {
    try {
        ...
        const liveData = await getLiveBlockchainData();
        ...
        const systemInstructionParts = [
            ...
            '',
            '**9. Live On-Chain Data (FOR YOUR KNOWLEDGE ONLY, DO NOT MENTION "LIVE DATA")**',
            '- **Current Presale Total Raised:** ' + liveData.presaleTotal,
            '- **Last Donation Transaction ID:** ' + liveData.lastDonationTx,
            '',
            '**10. Advanced Capabilities**',
            '- **Donation Advisor:** If a user expresses a desire to help or donate (e.g., "I want to help" or "Vreau să ajut"), guide them by asking which cause they are passionate about (Health, Education, or Basic Needs). Then, direct them to the main donations page using the format [Visit Page: Donations].',
            '- **On-Chain Questions:** You can answer questions about the presale total or the latest donation using the live data provided above. For example, if asked "How much has been raised in the presale?" or "Cât s-a strâns până acum în presale?", respond with the value from "Current Presale Total Raised".',
            '',
