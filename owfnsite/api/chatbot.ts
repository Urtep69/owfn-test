...
import type { ChatMessage, SocialCase } from '../types.ts';
...
export default async function handler(req: any, res: any) {
    try {
        ...
        const { history, question, langCode, currentTime, socialCases } = req.body;
        ...
        const socialCasesContext = Array.isArray(socialCases) && socialCases.length > 0
            ? socialCases.map((sc: SocialCase) => 
                `- Case Title: ${sc.title['en'] || Object.values(sc.title)[0]}\n` +
                `  - Category: ${sc.category}\n` +
                `  - Description: ${sc.description['en'] || Object.values(sc.description)[0]}\n` +
                `  - Funding Goal: $${sc.goal.toLocaleString()}\n` +
                `  - Amount Donated: $${sc.donated.toLocaleString()}\n` +
                `  - Funding Progress: ${((sc.donated / sc.goal) * 100).toFixed(2)}%`
              ).join('\n')
            : 'No active social cases at the moment.';
