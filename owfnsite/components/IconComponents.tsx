...
import { solanaLogo, usdcLogo, usdtLogo } from '../lib/assets.js';

export const AnimatedOwfnLogo = ({ className = 'w-12 h-12' }: { className?: string }) => {
    return (
        <div className={`relative flex items-center justify-center ${className}`} style={{ perspective: '1000px', transformStyle: 'preserve-3d' }}>
            <img 
                src="/assets/globe.png"
                alt="Rotating Globe"
                className="absolute w-full h-full object-cover animate-rotate-globe"
            />
            <img 
                src="/assets/owfn-text.png"
                alt="OWFN Logo Text"
...
export const OwfnIcon = ({ className = 'w-6 h-6' }: { className?: string }) => (
  <div className={`${className} rounded-full border-2 border-accent-600 dark:border-darkAccent-700 overflow-hidden`}>
    <AnimatedOwfnLogo className="w-full h-full" />
  </div>
);
