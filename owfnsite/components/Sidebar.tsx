...
const NavItem = ({ to, icon, label, isOpen, onClick }: { to: string, icon: React.ReactNode, label: string, isOpen: boolean, onClick: () => void }) => {
    ...
    const navLinkClasses = `flex items-center py-3 px-4 my-1 rounded-lg transition-colors duration-200 ${
        isActive 
            ? 'bg-accent/10 text-accent dark:bg-dark-accent/10 dark:text-dark-accent font-semibold' 
            : 'text-secondary dark:text-dark-secondary hover:bg-gray-200 dark:hover:bg-dark-surface'
        }`;
...
const NavGroup = ({ title, isOpen, children }: { title: string, isOpen: boolean, children: React.ReactNode }) => (
    <div>
        <h3 className={`px-4 pt-4 pb-2 text-xs font-semibold uppercase text-secondary dark:text-dark-secondary/70 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 hidden'}`}>
            {title}
        </h3>
        {children}
    </div>
...
export const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
    ...
    return (
        <aside className={`fixed top-0 left-0 h-full bg-surface/80 dark:bg-dark-surface/80 shadow-lg z-50 flex flex-col transition-all duration-300 ease-in-out backdrop-blur-lg border-r border-gray-200/20 dark:border-dark-border ${isOpen ? 'w-64' : 'w-20'} md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
             <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200/20 dark:border-dark-border flex-shrink-0">
                <Link to="/" onClick={handleLinkClick} className={`flex items-center space-x-3 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 md:opacity-100'}`}>
                    ...
                    <span className={`font-bold text-xl text-primary dark:text-dark-primary ${isOpen ? 'inline' : 'hidden'}`}>OWFN</span>
                </Link>
             </div>
            ...
            <div className="px-2 py-4 border-t border-gray-200/20 dark:border-dark-border flex-shrink-0">
