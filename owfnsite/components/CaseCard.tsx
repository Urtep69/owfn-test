...
export const CaseCard = ({ socialCase }: { socialCase: SocialCase }) => {
    ...
    return (
        <div className="glassmorphism rounded-lg overflow-hidden flex flex-col transition-all duration-300 hover:shadow-glow-md">
            <img src={socialCase.imageUrl} alt={title} className="w-full h-48 object-cover" />
            <div className="p-6 flex-grow flex flex-col">
                <span className="text-sm font-semibold text-accent mb-1">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                ...
                <Link to={`/impact/case/${socialCase.id}`} className="mt-auto block w-full text-center bg-accent text-accent-foreground font-bold py-2 rounded-lg hover:bg-accent-hover transition-colors">
                    {t('view_case')}
                </Link>
            </div>
        </div>
