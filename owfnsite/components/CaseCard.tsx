...
export const CaseCard = ({ socialCase }: { socialCase: SocialCase }) => {
    ...
    return (
        <div className="bg-white dark:bg-darkPrimary-800 rounded-lg shadow-3d overflow-hidden flex flex-col transition-transform transform hover:scale-105 duration-300">
            <Link to={`/impact/case/${socialCase.id}`}>
                <a className="block">
            ...
            <div className="p-6 flex-grow flex flex-col">
                ...
                <h3 className="text-xl font-bold mb-2 flex-grow min-h-[4rem]">{title}</h3>
                <p className="text-primary-600 dark:text-darkPrimary-400 mb-4 h-24 overflow-hidden">{description.substring(0, 100)}{description.length > 100 ? '...' : ''}</p>
