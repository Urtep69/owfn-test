...
const renderMessageContent = (text: string) => {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        if (match[1] === 'Visit Page') {
            ...
            if (path) {
                result.push(
                    <Link key={match.index} href={path} className="text-primary-600 dark:text-primary-400 font-bold underline hover:opacity-80">
                        {pageName}
                    </Link>
                );
            } else {
            ...
            if (url && platformName) {
                 result.push(
                    <a key={match.index} href={url} target="_blank" rel="noopener noreferrer" className="text-primary-600 dark:text-primary-400 font-bold underline hover:opacity-80 inline-flex items-center gap-1.5">
                        {icon} {platformName}
                    </a>
                );
            } else {
...
export const Chatbot = () => {
    ...
    const containerClasses = isMaximized
        ? "fixed inset-0 flex flex-col bg-slate-100/50 dark:bg-slate-900/50 backdrop-blur-2xl z-50 animate-fade-in-up"
        : "fixed bottom-5 right-5 w-full max-w-sm h-full max-h-[70vh] flex flex-col bg-slate-100/50 dark:bg-slate-900/50 backdrop-blur-2xl border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl animate-slide-in z-50";

    return (
        <div className={containerClasses} style={{ animationDuration: isMaximized ? '200ms' : '500ms' }}>
            <header className="flex items-center justify-between p-4 bg-primary-600 text-white rounded-t-2xl">
                        ...
                        <div key={index}>
                            <div className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className="flex flex-col">
                                    {msg.timestamp && (
                                        <p className={`text-xs text-slate-400 dark:text-slate-500 mb-1 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                                            {formatTimestamp(msg.timestamp)}
                                        </p>
                                    )}
                                    <div className={`max-w-xs md:max-w-sm px-4 py-2 rounded-xl ${msg.role === 'user' ? 'bg-primary-500 text-white rounded-br-none' : 'bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-200 rounded-bl-none'}`}>
                                       <div className="text-sm whitespace-pre-wrap">
                                           {msg.role === 'model' ? renderMessageContent(msg.parts[0].text) : msg.parts[0].text}
                                       </div>
                                    </div>
                                </div>
                                {msg.role === 'user' && <User className="w-6 h-6 text-primary-500 dark:text-primary-400 flex-shrink-0 mt-1" />}
                            </div>
                        </div>
                    ))}
                    {isLoading && (
                         <div className="flex items-start gap-3 justify-start">
                            ...
                            <div className="max-w-xs md:max-w-sm px-4 py-2 rounded-xl bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-200 rounded-bl-none">
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700">
                <div className="relative">
                    <input
                        ...
                        className="w-full p-3 pr-20 bg-slate-200 dark:bg-slate-700 rounded-lg focus:ring-2 focus:ring-primary-500 focus:outline-none"
                        disabled={isLoading}
                    />
                    <button
                        onClick={handleSend}
                        disabled={isLoading || input.trim() === ''}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-primary-600 text-white p-2 rounded-md hover:bg-primary-700 disabled:bg-slate-300 dark:disabled:bg-slate-600 disabled:cursor-not-allowed"
