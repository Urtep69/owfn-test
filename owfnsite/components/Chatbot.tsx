...
export const Chatbot = () => {
    ...
    const proactiveMessageSent = useRef<Set<string>>(new Set());

    const owfnBalance = solana.userTokens.find(t => t.symbol === 'OWFN')?.balance ?? 0;

    const getProactiveMessage = useCallback((path: string): string | null => {
        if (path === '/presale') return t('proactive_presale', { defaultValue: 'Hello! Do you have any questions about how the presale works or how to participate? I can help!' });
        if (path === '/donations') return t('proactive_donations', { defaultValue: 'Welcome to the donations page! Would you like to know which tokens are accepted or how to contribute to our cause?' });
        if (path === '/staking') return t('proactive_staking', { defaultValue: 'Interested in staking? I can explain how you can earn rewards with your OWFN tokens.' });
        return null;
    }, [t]);

    useEffect(() => {
        if (isOpen && messages.length === 0 && !proactiveMessageSent.current.has(location)) {
            const proactiveMessage = getProactiveMessage(location);
            if (proactiveMessage) {
                setTimeout(() => {
                    setMessages([{ role: 'model', parts: [{ text: proactiveMessage }], timestamp: new Date() }]);
                    proactiveMessageSent.current.add(location);
                }, 500);
            }
        }
    }, [isOpen, location, messages.length, getProactiveMessage]);
