    ...
    if (!connected) {
        return (
            ...
            >
                ...
                <span className="relative flex items-center gap-2"><WalletManagerIcon className="w-5 h-5" /> {t('connect_wallet')}</span>
            </button>
        );
    }
        ...
        return (
            ...
            >
                ...
                <span className="relative flex items-center gap-2">
                    ...
                    {isSiwsLoading ? "Verifying..." : "Sign-In with Solana"}
                </span>
            </button>
        );
    }
    ...
    return (
         <div className="relative" ref={dropdownRef}>
            <button
                ...
                className="flex items-center bg-primary-200 dark:bg-darkPrimary-800 border border-primary-300 dark:border-darkPrimary-700 rounded-full text-sm font-semibold text-primary-800 dark:text-darkPrimary-100 transition-all duration-300 hover:border-accent-500/70 dark:hover:border-darkAccent-400/70 shadow-lg px-4 py-2"
            >
                <div className="flex items-center space-x-2">
                    <WalletManagerIcon className="w-5 h-5" />
                    <span className="font-mono">{truncateAddress(address || '')}</span>
                    <ChevronRight size={16} className={`transition-transform ${isDropdownOpen ? 'rotate-90' : ''}`} />
                </div>
            </button>
