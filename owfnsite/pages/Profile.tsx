...
import { Wallet, DollarSign, HandHeart, Vote, Award, ShieldCheck, Gem, Loader2, KeyRound } from 'lucide-react';
...
export default function Profile() {
    ...
    // FIX: Destructured userStats from the solana object to resolve reference errors.
    const { connected, address, userTokens, loading: tokensLoading, userStats } = solana;
    const { isAuthenticated, session, isSessionLoading, isLoading: isAuthLoading, signIn } = siws;
    ...
    if (isSessionLoading || solana.connecting) {
        return (
    ...
    if (!isAuthenticated || !session) {
        return (
            <div className="text-center p-12 bg-white dark:bg-darkPrimary-800 rounded-lg shadow-3d animate-fade-in-up">
                ...
                <p className="text-primary-600 dark:text-darkPrimary-400 mb-6">{t(connected ? 'profile_sign_in_prompt' : 'profile_connect_prompt')}</p>
                <button
                    onClick={connected ? signIn : () => setWalletModalOpen(true)}
    ...
    const loading = tokensLoading || isAuthLoading;
