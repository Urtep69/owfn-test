...
import { Wallet, DollarSign, HandHeart, Vote, Award, ShieldCheck, Gem, Loader2, LogIn, Image as ImageIcon } from 'lucide-react';
...
const MOCK_NFTS: ImpactNFT[] = [
    { id: 'nft1', caseId: '1', caseTitle: 'School Rebuilt in Romania', imageUrl: `https://picsum.photos/seed/nft1/400/400`, date: '2026-03-15'},
    { id: 'nft2', caseId: '2', caseTitle: 'Hospital Wing Modernized', imageUrl: `https://picsum.photos/seed/nft2/400/400`, date: '2026-05-20'},
];
    ...
    if (connected && !isAuthenticated) {
        return (
            <div className="text-center p-12 bg-white dark:bg-darkPrimary-800 rounded-lg shadow-3d animate-fade-in-up">
                ...
                <h1 className="text-2xl font-bold mb-2">{t('verify_your_wallet', { defaultValue: 'Verify Your Wallet' })}</h1>
                <p className="text-primary-600 dark:text-darkPrimary-400 mb-6">{t('profile_sign_in_prompt', { defaultValue: 'Sign in with your wallet to access your personalized Impact Dashboard and see your contributions.' })}</p>
                <button
                    onClick={signIn}
                    disabled={isSiwsLoading}
                    className="bg-accent-400 hover:bg-accent-500 text-accent-950 dark:bg-darkAccent-500 dark:hover:bg-darkAccent-600 dark:text-darkPrimary-950 font-bold py-3 px-6 rounded-lg transition-colors duration-300 disabled:opacity-50"
                >
                    {isSiwsLoading ? t('authenticating', { defaultValue: 'Authenticating...' }) : t('sign_in_to_verify', { defaultValue: 'Sign In to Verify' })}
                </button>
            </div>
        );
    }
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-darkPrimary-800 p-6 rounded-lg shadow-3d">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                       {MOCK_NFTS.map(nft => (
                           <div key={nft.id} className="group relative rounded-lg overflow-hidden border-2 border-primary-200 dark:border-darkPrimary-700">
                               <img src={nft.imageUrl} alt={nft.caseTitle} className="w-full h-full object-cover aspect-square" />
                               <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent"></div>
                               <div className="absolute bottom-0 left-0 p-2 text-white">
                                   <p className="font-bold text-xs leading-tight">{nft.caseTitle}</p>
                                   <p className="text-xs opacity-80">{nft.date}</p>
                               </div>
                           </div>
                       ))}
                       {MOCK_NFTS.length === 0 && (
                           <div className="col-span-full text-center py-8 text-primary-500 dark:text-darkPrimary-400">
                                <ImageIcon className="mx-auto w-10 h-10 mb-2"/>
                                <p>No Impact NFTs yet.</p>
                           </div>
                       )}
                    </div>
                </div>
