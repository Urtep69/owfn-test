...
const StatCard = ({ icon, title, value }: { icon: React.ReactNode, title: string, value: string | number }) => (
    <div className="bg-background border border-border p-4 rounded-lg flex items-center space-x-4">
...
export default function Profile() {
    ...
    if (!connected) {
        return (
            <div className="text-center p-12 bg-surface border border-border rounded-lg animate-fade-in-up">
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div>
                <h1 className="text-4xl font-bold text-primary">{t('impact_dashboard_title')}</h1>
            ...
            <div className="bg-surface border border-border p-6 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-background border border-border rounded-lg">
                    <div>
                    ...
                    <div className="space-y-2">
                        ...
                        {userTokens.map(token => (
                           <Link key={token.mintAddress} to={`/dashboard/token/${token.mintAddress}?from=/profile`}>
                                <a className="grid grid-cols-3 gap-4 items-center p-4 rounded-lg hover:bg-background transition-colors duration-200 cursor-pointer">
                                    {/* Column 1: Asset Info */}
                                    ...
                                    <div className="text-right font-mono">
                                        <p className="font-semibold text-text-primary">{formatNumber(token.balance)}</p>
                         ...
                         <div className="flex flex-wrap gap-4">
                            {MOCK_BADGES.map(badge => (
                                 <div key={badge.id} className="group relative flex flex-col items-center text-center w-24">
                                    <div className="bg-background border border-border rounded-full p-4 text-primary group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="absolute bottom-full mb-2 w-48 bg-text-primary text-background text-xs rounded py-1 px-2 text-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
