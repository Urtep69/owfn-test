...
import { Wallet, DollarSign, HandHeart, Vote, Award, ShieldCheck, Gem, Loader2, CalendarDays, History, Zap } from 'lucide-react';
...
export default function Profile() {
    ...
    const { connected, address, userTokens, loading, userStats, onChainStats } = solana;
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            ...
            <div className="bg-white dark:bg-darkPrimary-800 p-6 rounded-lg shadow-3d">
                <h2 className="text-2xl font-bold mb-4">{t('on_chain_snapshot_title')}</h2>
                {onChainStats.isLoading ? (
                    <div className="flex justify-center items-center py-8 text-primary-600 dark:text-darkPrimary-400 gap-3">
                        <Loader2 className="w-8 h-8 animate-spin" />
                        <span>Calculating on-chain snapshot...</span>
                    </div>
                ) : (
                    <div className="grid md:grid-cols-3 gap-4">
                        <StatCard icon={<CalendarDays size={24} />} title={t('wallet_age_title')} value={onChainStats.walletAge || 'N/A'} />
                        <StatCard icon={<History size={24} />} title={t('total_transactions_title')} value={onChainStats.totalTransactions >= 1000 ? `${onChainStats.totalTransactions}+` : onChainStats.totalTransactions} />
                        <StatCard icon={<Zap size={24} />} title={t('total_fees_title')} value={`${onChainStats.totalFees.toFixed(6)} SOL`} />
                    </div>
                )}
            </div>
