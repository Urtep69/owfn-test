...
const StatCard = ({ icon, title, value }: { icon: React.ReactNode, title: string, value: string | number }) => (
    <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg flex items-center space-x-4">
...
export default function Profile() {
    ...
    if (!connected) {
        return (
            <div className="text-center p-12 bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg animate-fade-in-up">
                ...
                <p className="text-slate-600 dark:text-slate-400 mb-6">{t('profile_connect_prompt')}</p>
                <button
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div>
                <h1 className="text-4xl font-bold text-primary-600 dark:text-primary-400">{t('impact_dashboard_title')}</h1>
                <div className="flex items-center space-x-2 mt-2">
                    <span className="text-sm text-slate-600 dark:text-slate-400">{t('connected_as')}:</span>
            ...
            <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                    <div>
                        <p className="text-sm text-slate-600 dark:text-slate-400">{t('token_types')}</p>
                ...
                {loading ? (
                    <div className="text-center py-8 text-slate-600 dark:text-slate-400 flex items-center justify-center gap-3">
                    ...
                    <div className="space-y-2">
                        {/* Header */}
                        <div className="grid grid-cols-3 gap-4 px-4 py-2 text-xs text-slate-500 font-bold uppercase">
                            <span>{t('asset')}</span>
                        ...
                        {userTokens.map(token => (
                           <Link key={token.mintAddress} to={`/dashboard/token/${token.mintAddress}?from=/profile`}>
                                <a className="grid grid-cols-3 gap-4 items-center p-4 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors duration-200 cursor-pointer">
                                    {/* Column 1: Asset Info */}
                                    ...
                                    <div className="text-right font-mono">
                                        <p className="font-semibold text-slate-900 dark:text-slate-100">{formatNumber(token.balance)}</p>
                                        <p className="text-sm text-slate-600 dark:text-slate-400">@ ${token.pricePerToken > 0.01 ? token.pricePerToken.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) : token.pricePerToken.toPrecision(4)}</p>
                                    </div>

                                    {/* Column 3: Value */}
                                    <div className="text-right font-semibold font-mono text-slate-900 dark:text-slate-100">
                         ...
                         <div className="flex flex-wrap gap-4">
                            {MOCK_BADGES.map(badge => (
                                 <div key={badge.id} className="group relative flex flex-col items-center text-center w-24">
                                    <div className="bg-slate-100 dark:bg-slate-700 rounded-full p-4 text-primary-500 dark:text-primary-400 group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="absolute bottom-full mb-2 w-48 bg-slate-900 text-white dark:bg-slate-950 text-xs rounded py-1 px-2 text-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                        {t(badge.descriptionKey)}
                                        <svg className="absolute text-slate-900 dark:text-slate-950 h-2 w-full left-0 top-full" x="0px" y="0px" viewBox="0 0 255 255"><polygon className="fill-current" points="0,0 127.5,127.5 255,0"/></svg>
                                    </div>
                                </div>
                            ))}
                        </div>
