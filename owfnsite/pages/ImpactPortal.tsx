...
const AdminPortal = ({ onAddCase }: { onAddCase: (newCase: SocialCase) => void }) => {
    ...
    return (
        <div className="bg-surface border border-border p-8 rounded-lg mt-12">
            <h2 className="text-3xl font-bold mb-6 text-primary">{t('create_new_case')}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
                <input type="text" placeholder={t('case_title')} value={title} onChange={e => setTitle(e.target.value)} required className="w-full p-2 bg-background border border-border rounded-md" />
                <textarea placeholder={t('case_description')} value={description} onChange={e => setDescription(e.target.value)} required className="w-full p-2 bg-background border border-border rounded-md h-32"></textarea>
                <textarea placeholder={t('case_details')} value={details} onChange={e => setDetails(e.target.value)} required className="w-full p-2 bg-background border border-border rounded-md h-24"></textarea>
                <input type="text" placeholder={t('image_url')} value={imageUrl} onChange={e => setImageUrl(e.target.value)} className="w-full p-2 bg-background border border-border rounded-md" />
                <div className="grid grid-cols-2 gap-4">
                    <select value={category} onChange={e => setCategory(e.target.value)} className="w-full p-2 bg-background border border-border rounded-md">
                    ...
                    <input type="number" placeholder={t('funding_goal_usd')} value={goal} onChange={e => setGoal(e.target.value)} required className="w-full p-2 bg-background border border-border rounded-md" min="1" />
                </div>
                <button type="submit" disabled={isTranslating} className="w-full bg-primary text-white py-3 rounded-lg font-bold hover:bg-opacity-80 disabled:bg-border disabled:cursor-not-allowed transition-colors">
        ...
        <div className="animate-fade-in-up space-y-8">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                {categories.map(category => (
                    ...
                    >
                       <a className="block text-center p-8 bg-surface border border-border rounded-lg hover:border-primary hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary">
                            {category.icon}
