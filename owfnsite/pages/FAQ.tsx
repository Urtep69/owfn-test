...
const FaqItem = ({ question, answer, isOpen, onClick }: { question: string, answer: string, isOpen: boolean, onClick: () => void }) => {
    return (
        <div className="border-b-2 border-primary-900 dark:border-primary-200 py-4 last:border-b-0">
            <button
                onClick={onClick}
                className="w-full flex justify-between items-center text-left text-lg font-semibold text-primary-800 dark:text-primary-200"
            >
                <span>{question}</span>
            ...
            >
                <p className="text-primary-600 dark:text-primary-400 leading-relaxed pt-2 whitespace-pre-wrap">
                    {answer}
                </p>
            </div>
        </div>
    ...
    return (
        <div className="animate-fade-in-up max-w-4xl mx-auto space-y-8">
            <div className="text-center">
                <HelpCircle className="mx-auto w-16 h-16 text-accent-500 mb-4" />
                <h1 className="text-4xl font-bold text-accent-600 dark:text-darkAccent-500">{t('faq_title')}</h1>
            ...
            <div className="space-y-4">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-primary-50 dark:bg-darkPrimary-800 border-2 border-primary-900 dark:border-primary-200 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent focus:outline-none transition-colors"
                    />
                </div>
                <div className="flex flex-wrap gap-2">
                    {categories.map(cat => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors duration-200 border-2 border-primary-900 dark:border-primary-200 ${
                                activeCategory === cat.key
                                    ? 'bg-accent-500 text-white'
                                    : 'bg-primary-100 text-primary-700 hover:bg-primary-200 dark:bg-darkPrimary-700 dark:text-primary-300 dark:hover:bg-darkPrimary-600'
                            }`}
                        >
                            {t(cat.nameKey)}
                        </button>
            ...
            <div className="bg-primary-50 dark:bg-darkPrimary-800 p-8 rounded-lg shadow-neo-brutal dark:shadow-dark-neo-brutal border-2 border-primary-900 dark:border-primary-100">
