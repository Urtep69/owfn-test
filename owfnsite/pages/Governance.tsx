...
const ProposalCard = ({ proposal }: { proposal: GovernanceProposal }) => {
    ...
    const hasVoted = userStats.votedProposalIds.includes(String(proposal.id));
    ...
    return (
        <div className="bg-white dark:bg-darkPrimary-800 p-6 rounded-lg shadow-3d space-y-4 flex flex-col">
            <div className="flex justify-between items-start">
                <h3 className="text-xl font-bold pr-2">{title}</h3>
                {getStatusChip()}
            </div>
            <p className="text-primary-600 dark:text-darkPrimary-400 text-sm flex-grow">{description}</p>
            <div className="text-xs text-primary-500 dark:text-darkPrimary-500">Proposed by: <AddressDisplay address={proposal.proposer_address} /></div>
            
            <div className="space-y-2">
                <div className="w-full bg-red-500/30 rounded-full h-4 flex overflow-hidden">
                    <div className="bg-green-500/80 h-full" style={{ width: `${forPercentage}%` }}></div>
                </div>
                <div className="flex justify-between text-sm font-semibold">
                    <span className="text-green-600 dark:text-green-400">{t('votes_for')}: {forPercentage.toFixed(2)}% ({proposal.votes_for.toLocaleString()})</span>
                    <span className="text-red-600 dark:text-red-400">{t('votes_against')}: {(100 - forPercentage).toFixed(2)}% ({proposal.votes_against.toLocaleString()})</span>
                </div>
            </div>
            
            {proposal.status === 'active' && (
                <div className="flex justify-between items-center border-t border-primary-200 dark:border-darkPrimary-700 pt-4">
                    <div className="text-sm text-primary-600 dark:text-darkPrimary-400">{t('ends_in')}: <Countdown endDateString={proposal.end_date} /></div>
                    {connected && (
                        hasVoted ? (
...
export default function Governance() {
    ...
    const handleCreateProposal = async (e: React.FormEvent) => {
        ...
        await addProposal({ title: newTitle, description: newDescription });
                    ...
                    <div className="bg-white dark:bg-darkPrimary-800 rounded-lg shadow-xl p-8 w-full max-w-2xl relative animate-fade-in-up" style={{ animationDuration: '300ms' }}>
                        ...
                        <form onSubmit={handleCreateProposal} className="space-y-4">
                            <div>
                                ...
                                <input id="prop-title" type="text" value={newTitle} onChange={e => setNewTitle(e.target.value)} required className="w-full p-2 bg-primary-100 dark:bg-darkPrimary-700 rounded-md" />
                            </div>
                            <div>
                                <label htmlFor="prop-desc" className="block text-sm font-medium mb-1">{t('proposal_description')}</label>
                                <textarea id="prop-desc" value={newDescription} onChange={e => setNewDescription(e.target.value)} required rows={5} className="w-full p-2 bg-primary-100 dark:bg-darkPrimary-700 rounded-md"></textarea>
                            </div>
                            <button type="submit" disabled={solana.loading} className="w-full bg-accent-400 text-accent-950 dark:bg-darkAccent-500 dark:text-darkPrimary-950 py-3 rounded-lg font-bold hover:bg-accent-500 dark:hover:bg-darkAccent-600 disabled:opacity-50 flex justify-center items-center gap-2">
                                {solana.loading && <Loader2 className="animate-spin" />}
                                {solana.loading ? t('processing') : t('submit_proposal')}
                            </button>
                        </form>
                    </div>
                </div>
