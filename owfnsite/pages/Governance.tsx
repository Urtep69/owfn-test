...
const ProposalCard = ({ proposal }: { proposal: GovernanceProposal }) => {
    ...
    const getStatusChip = () => {
        switch(proposal.status) {
            ...
            case 'passed': return <div className="bg-secondary/20 text-secondary text-xs font-bold px-2 py-1 rounded-full">{t('status_passed')}</div>;
            case 'failed': return <div className="bg-destructive/20 text-destructive text-xs font-bold px-2 py-1 rounded-full">{t('status_failed')}</div>;
        }
    }
        ...
        <div className="bg-surface border border-border p-6 rounded-lg space-y-4">
            ...
            <div className="space-y-2">
                <div className="w-full bg-border rounded-full h-4 flex overflow-hidden">
                    <div className="bg-secondary h-full" style={{ width: `${forPercentage}%` }}></div>
                    <div className="bg-destructive h-full" style={{ width: `${againstPercentage}%` }}></div>
                </div>
                <div className="flex justify-between text-sm font-semibold">
                    <span className="text-secondary">{t('votes_for')}: {forPercentage.toFixed(2)}%</span>
                    <span className="text-destructive">{t('votes_against')}: {againstPercentage.toFixed(2)}%</span>
                </div>
            </div>
                    ...
                    {connected && (
                        hasVoted ? (
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => handleVote('for')} disabled={loading} className="bg-secondary text-black font-bold py-2 px-4 rounded-lg hover:bg-opacity-80 disabled:opacity-50"><ThumbsUp size={16}/></button>
                                <button onClick={() => handleVote('against')} disabled={loading} className="bg-destructive text-white font-bold py-2 px-4 rounded-lg hover:bg-opacity-80 disabled:opacity-50"><ThumbsDown size={16}/></button>
                            </div>
                        )
                    )}
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                {solana.connected && (
                    <button 
                        ...
                        className="flex items-center gap-2 bg-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-opacity-80 transition-colors"
                    >
                    ...
                    <div className="bg-surface border border-border rounded-lg shadow-xl p-8 w-full max-w-2xl relative animate-fade-in-up" style={{ animationDuration: '300ms' }}>
                        ...
                        <form onSubmit={handleCreateProposal} className="space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-primary text-white py-3 rounded-lg font-bold hover:bg-opacity-80 disabled:opacity-50"
                            >
