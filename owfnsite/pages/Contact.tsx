    ...
    return (
        <div className="bg-surface border border-border p-6 rounded-lg flex flex-col">
            <div className="flex items-center space-x-4 mb-4">
                <div className="flex-shrink-0 flex items-center justify-center w-12 h-12 bg-background rounded-lg text-primary">
                    {icon}
                </div>
                <div>
                    <h3 className="text-xl font-bold text-text-primary">{title}</h3>
                    <p className="text-text-secondary break-all">{email}</p>
                </div>
            </div>
            <p className="text-text-secondary flex-grow mb-6">{description}</p>
            <button 
                onClick={() => onButtonClick(reasonKey)}
                className="mt-auto bg-primary hover:bg-opacity-80 text-white font-bold py-2.5 px-5 rounded-lg transition-transform transform hover:scale-105 w-full flex items-center justify-center gap-2"
            >
                <Mail size={18} />
...
const SocialLinkCard = ({ icon, title, description, href }: { icon: React.ReactNode, title: string, description: string, href: string }) => (
    <a
        ...
        className="block bg-surface border border-border p-6 rounded-lg hover:border-primary hover:-translate-y-1 transition-all duration-300 transform text-center"
    >
...
export default function Contact() {
    ...
    return (
        <div className="animate-fade-in-up space-y-12">
            ...
            <section ref={formRef} className="bg-surface border border-border p-8 rounded-lg scroll-mt-24">
                <h2 className="text-3xl font-bold text-center mb-8">{t('contact_form_title')}</h2>
                {status === 'success' ? (
                    <div className="text-center p-8 bg-secondary/10 rounded-lg animate-fade-in-up">
                        <CheckCircle className="mx-auto w-16 h-16 text-secondary mb-4" />
                        <h3 className="text-2xl font-bold text-secondary">{t('contact_success_title')}</h3>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl mx-auto">
                        ...
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium text-text-secondary">{t('contact_form_name')} <span className="text-destructive">*</span></label>
                            <input type="text" id="name" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
                        </div>
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-text-secondary">{t('contact_form_email')} <span className="text-destructive">*</span></label>
                            <input type="email" id="email" value={email} onChange={e => setEmail(e.target.value)} required className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
                        </div>
                         <div>
                            <label htmlFor="reason" className="block text-sm font-medium text-text-secondary">{t('contact_form_reason')} <span className="text-destructive">*</span></label>
                            <select id="reason" value={reason} onChange={e => setReason(e.target.value)} required className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary">
                                {reasonOptions.map(opt => <option key={opt.key} value={opt.key}>{t(opt.labelKey)}</option>)}
                            </select>
                        </div>
                        <div>
                            <label htmlFor="message" className="block text-sm font-medium text-text-secondary">{t('contact_form_message')} <span className="text-destructive">*</span></label>
                            <textarea id="message" value={message} onChange={e => setMessage(e.target.value)} required rows={5} className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary"></textarea>
                        </div>
                        {status === 'error' && (
                            <p className="text-destructive text-sm text-center">{t('contact_error_message')}</p>
                        )}
                        <div>
                            <button type="submit" disabled={status === 'loading'} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-bold text-white bg-primary hover:bg-opacity-80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <section>
                <div className="bg-surface p-6 rounded-lg shadow-md flex items-start gap-4 border border-border">
            ...
            <section className="bg-primary/10 p-8 rounded-lg border border-primary/20 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <Link to="/faq" className="bg-primary text-white font-bold py-3 px-8 rounded-lg text-lg hover:bg-opacity-80 transition-transform transform hover:scale-105 shadow-lg flex-shrink-0">
