    ...
    return (
        <div className="bg-background border border-border rounded-lg p-4 h-full flex flex-col">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-foreground font-bold">{t('live_presale_feed')}</h3>
            </div>
            <div className="grid grid-cols-4 gap-2 text-xs text-foreground-muted pb-2 border-b border-border font-semibold">
            ...
            <div className="flex-grow overflow-y-auto space-y-1 pr-1 -mr-2 mt-2">
                {loading ? (
                    ...
                    <div key={tx.id} className={`grid grid-cols-4 gap-2 items-center text-sm p-1.5 rounded-md animate-fade-in-up ${tx.time.getTime() > Date.now() - 10000 ? 'bg-primary/10' : ''}`}>
...
const AccordionSection = ({ title, children, isOpen: defaultIsOpen = false }: { title: string, children: React.ReactNode, isOpen?: boolean }) => {
  ...
  return (
    <div className="border border-border bg-surface/30 rounded-lg">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex justify-between items-center p-4"
      >
        <h3 className="font-bold text-md text-foreground">{title}</h3>
        <ChevronDown className={`w-5 h-5 text-foreground-muted transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      {isOpen && (
        <div className="px-4 pb-4 text-foreground-muted animate-fade-in-up" style={{animationDuration: '300ms'}}>
          {children}
        </div>
      )}
    </div>
...
const ProjectInfoRow = ({ label, value }: { label: string, value: React.ReactNode }) => (
  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3 border-b border-border/50">
    <span className="text-foreground-muted mb-1 sm:mb-0">{label}</span>
    <div className="font-semibold text-foreground text-left sm:text-right break-all w-full sm:w-auto">{value}</div>
  </div>
);
...
export default function Presale() {
  ...
  return (
    <div className="bg-background text-foreground min-h-screen -m-8 p-4 md:p-8 flex justify-center font-sans">
      <div className="w-full max-w-screen-2xl">
        ...
        <div className="bg-surface rounded-xl p-6 md:p-10 border border-border shadow-lg">
            
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <img src={OWFN_LOGO_URL} alt="Token Logo" className="w-20 h-20 rounded-full border-2 border-primary"/>
                <div className="flex-grow">
                    <h1 className="text-2xl font-bold text-foreground">{t('presale_join_title')}</h1>
                    <h2 className="text-lg text-foreground-muted">{t('presale_header_subtitle')}</h2>
                </div>
                <div className="flex items-center gap-3 text-foreground-muted">
                    <a href={PROJECT_LINKS.x} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full hover:bg-border transition-colors"><Twitter size={20}/></a>
                    <a href={PROJECT_LINKS.telegramGroup} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full hover:bg-border transition-colors"><Send size={20}/></a>
                    <a href={PROJECT_LINKS.website} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full hover:bg-border transition-colors"><Globe size={20}/></a>
                </div>
            </div>

            {/* Description */}
            <p className="text-foreground-muted text-sm leading-relaxed mt-4">{t('about_mission_desc')}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mt-6">
                ...
                <div className="lg:col-span-3 space-y-6">
                    ...
                    <div className="w-full">
                        ...
                        <div className="w-full bg-border rounded-full h-2.5">
                            <div className="bg-primary h-2.5 rounded-full" style={{width: `${saleProgress}%`}}></div>
                        </div>
                        <div className="flex justify-between mt-1 text-sm text-foreground">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-background border border-border rounded-lg p-4 text-center">
                            <p className="text-foreground-muted text-sm">{t('presale_whitelist_finished')}</p>
                            <p className="text-foreground text-2xl font-mono font-bold">--:--:--:--</p>
                        </div>
                        <div className="bg-background border border-border rounded-lg p-4 text-center">
                     ...
                     <div className="space-y-4">
                        <AccordionSection title={t('presale_project_info_title')} isOpen={true}>
                            <div className="space-y-1 text-sm">
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3 border-b border-border/50">
                                    <span className="text-foreground-muted mb-1 sm:mb-0">{t('presale_token_address_label')}</span>
                            ...
                            <div className="space-y-2">
                                {TOKEN_ALLOCATIONS.map(alloc => (
                                    <div key={alloc.name} className="flex items-center space-x-3">
                                        ...
                                        <span className="text-sm text-foreground-muted">{alloc.name} ({alloc.percentage}%)</span>
                                    </div>
                                ))}
                                <Link to="/tokenomics" className="text-primary hover:underline pt-2 inline-block">{t('view_full_details')}</Link>
                            </div>
                        </AccordionSection>
                            ...
                            <div className="space-y-3">
                                {ROADMAP_DATA.map(phase => (
                                    <div key={phase.key_prefix}>
                                        <h4 className="font-bold text-foreground">{t(`${phase.key_prefix}_title`)} ({phase.quarter})</h4>
                                        <p className="text-sm text-foreground-muted">{t(`${phase.key_prefix}_description`)}</p>
                                    </div>
                                ))}
                                <Link to="/roadmap" className="text-primary hover:underline pt-2 inline-block">{t('view_full_details')}</Link>
                            </div>
                        </AccordionSection>
                ...
                <div className="lg:col-span-2 space-y-6 flex flex-col">
                    ...
                    <div className="bg-background border border-border rounded-lg p-6 space-y-4">
                        ...
                        {solana.connected && (
                            <div className="text-center text-xs text-foreground-muted p-2 bg-surface/50 rounded-md">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className={`w-full bg-background border rounded-lg py-3 pl-11 pr-4 text-lg font-mono text-foreground text-right focus:ring-2 placeholder-foreground-muted/50 ${error ? 'border-red-500 focus:ring-red-500' : 'border-border focus:ring-ring focus:border-ring'}`}
                                placeholder="0.00"
                        ...
                        <div className="bg-surface/50 p-4 rounded-lg space-y-3">
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-foreground-muted">{t('owfn_base_amount')}</span>
                            ...
                            <div className="flex justify-between items-center text-lg">
                                <span className="font-bold text-foreground">{t('you_receive')}</span>
                                <div className="flex items-center gap-2">
                                    <OwfnIcon className="w-6 h-6"/>
                                    <span className="font-mono font-bold text-2xl text-primary">{calculation.total.toLocaleString(undefined, { maximumFractionDigits: 3 })}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <button 
                            onClick={handleBuy}
                            className="w-full bg-primary text-primary-foreground font-bold py-3 px-8 rounded-lg text-lg hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                            disabled={solana.loading || isCheckingContribution || (solana.connected && (isAmountInvalid || maxAllowedBuy <= 0 || presaleStatus !== 'active'))}
                        >
                            {buttonText}
                        </button>

                         <div className="bg-secondary/10 border border-secondary/30 p-3 rounded-lg text-center">
                            <p className="font-bold text-secondary flex items-center justify-center gap-2">
