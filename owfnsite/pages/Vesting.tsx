...
const NoScheduleInfo = () => {
    ...
    return (
        <div className="text-center p-12 glassmorphism rounded-lg">
            <Calendar className="mx-auto w-16 h-16 text-accent mb-4" />
    ...
    return (
        <div className="glassmorphism p-8 rounded-lg space-y-6">
            ...
            <div className="bg-accent/10 p-6 rounded-lg text-center">
                ...
                <p className="text-4xl font-bold my-2 text-accent">{claimableNow.toLocaleString(undefined, {maximumFractionDigits: 2})}</p>
                <button onClick={handleClaim} disabled={solana.loading || claimableNow <= 0} className="mt-4 w-full max-w-xs bg-accent text-accent-foreground font-bold py-3 rounded-lg text-lg hover:bg-accent-hover transition-colors disabled:opacity-50">
    ...
    return (
        <div className="mt-12 space-y-8">
            <div className="glassmorphism p-8 rounded-lg">
                <h2 className="text-2xl font-bold mb-6 flex items-center"><PlusCircle className="mr-3 text-accent"/>{t('create_vesting_schedule')}</h2>
                <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                    <input type="text" placeholder={t('recipient_address')} value={recipient} onChange={e => setRecipient(e.target.value)} required className="w-full p-3 bg-surface-2 rounded-md border border-border-color focus:ring-2 focus:ring-accent" />
                    <input type="number" placeholder={t('total_amount_owfn')} value={amount} onChange={e => setAmount(e.target.value)} required className="w-full p-3 bg-surface-2 rounded-md border border-border-color focus:ring-2 focus:ring-accent" />
                    <input type="number" placeholder={t('vesting_duration_months')} value={duration} onChange={e => setDuration(e.target.value)} required className="w-full p-3 bg-surface-2 rounded-md border border-border-color focus:ring-2 focus:ring-accent" />
                    <input type="number" placeholder={t('cliff_period_months')} value={cliff} onChange={e => setCliff(e.target.value)} required className="w-full p-3 bg-surface-2 rounded-md border border-border-color focus:ring-2 focus:ring-accent" />
                    <button type="submit" className="md:col-span-2 w-full bg-accent text-accent-foreground py-3 rounded-lg font-bold hover:bg-accent-hover transition-colors">{t('create_schedule')}</button>
                </form>
            </div>
            <div className="glassmorphism p-8 rounded-lg">
                 ...
                 <div className="space-y-4">
                     {vestingSchedules.map(schedule => (
                         <div key={schedule.recipientAddress} className="bg-surface-2 p-4 rounded-lg">
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="text-center">
                <h1 className="text-4xl font-display font-bold text-accent">{t('vesting_title')}</h1>
