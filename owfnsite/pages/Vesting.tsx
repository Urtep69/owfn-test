...
const ConnectWalletPrompt = () => {
    ...
    return (
        <div className="text-center p-12 bg-surface border border-border rounded-lg shadow-lg">
            <Wallet className="mx-auto w-16 h-16 text-primary mb-4" />
            <h2 className="text-2xl font-bold mb-2">{t('vesting_connect_title')}</h2>
            <p className="text-foreground-muted mb-6">{t('vesting_connect_prompt')}</p>
            <button
...
const NoScheduleInfo = () => {
    ...
    return (
        <div className="text-center p-12 bg-surface border border-border rounded-lg shadow-lg">
            ...
            <p className="text-foreground-muted">{t('no_vesting_schedule_desc')}</p>
        </div>
    );
}
...
const VestingScheduleDetails = ({ schedule }: { schedule: VestingSchedule }) => {
    ...
    return (
        <div className="bg-surface border border-border p-8 rounded-lg shadow-lg space-y-6">
            <h2 className="text-2xl font-bold">{t('my_vesting_schedule')}</h2>
            <div className="grid md:grid-cols-3 gap-4 text-center">
                <div className="bg-background/50 p-4 rounded-lg">
                    <p className="text-sm text-foreground-muted">{t('total_allocation')}</p>
                    <p className="text-xl font-bold">{schedule.totalAmount.toLocaleString()} OWFN</p>
                </div>
                <div className="bg-background/50 p-4 rounded-lg">
                    <p className="text-sm text-foreground-muted">{t('claimed')}</p>
                    <p className="text-xl font-bold">{claimedAmount.toLocaleString()} OWFN</p>
                </div>
                <div className="bg-background/50 p-4 rounded-lg">
                    <p className="text-sm text-foreground-muted">{t('remaining')}</p>
            ...
            <div className="bg-primary/10 p-6 rounded-lg text-center">
                ...
                <button onClick={handleClaim} disabled={solana.loading || claimableNow <= 0} className="mt-4 w-full max-w-xs bg-primary text-primary-foreground font-bold py-3 rounded-lg text-lg hover:bg-primary/90 transition-colors disabled:opacity-50">
...
const AdminPortal = () => {
    ...
    const formInputClasses = "w-full p-3 bg-background border border-border rounded-md";
        ...
        <div className="mt-12 space-y-8">
            <div className="bg-surface border border-border p-8 rounded-lg shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                    <input type="text" placeholder={t('recipient_address')} value={recipient} onChange={e => setRecipient(e.target.value)} required className={formInputClasses} />
                    <input type="number" placeholder={t('total_amount_owfn')} value={amount} onChange={e => setAmount(e.target.value)} required className={formInputClasses} />
                    <input type="number" placeholder={t('vesting_duration_months')} value={duration} onChange={e => setDuration(e.target.value)} required className={formInputClasses} />
                    <input type="number" placeholder={t('cliff_period_months')} value={cliff} onChange={e => setCliff(e.target.value)} required className={formInputClasses} />
                    <button type="submit" className="md:col-span-2 w-full bg-primary text-primary-foreground py-3 rounded-lg font-bold hover:bg-primary/90 transition-colors">{t('create_schedule')}</button>
                </form>
            </div>
            <div className="bg-surface border border-border p-8 rounded-lg shadow-lg">
                 ...
                 <div className="space-y-4">
                     {vestingSchedules.map(schedule => (
                         <div key={schedule.recipientAddress} className="bg-background p-4 rounded-lg">
                            <AddressDisplay address={schedule.recipientAddress}/>
                            <p className="text-sm text-foreground-muted">{t('total_allocation')}: {schedule.totalAmount.toLocaleString()} OWFN</p>
                            <p className="text-sm text-foreground-muted">{t('end_date')}: {new Intl.DateTimeFormat('en-CA').format(schedule.endDate)}</p>
                         </div>
                     ))}
                 </div>
            </div>
