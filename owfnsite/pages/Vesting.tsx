...
const NoScheduleInfo = () => {
    ...
    return (
        <div className="text-center p-12 glass-card max-w-md mx-auto card-3d-hover">
            <Calendar className="mx-auto w-16 h-16 text-neon-cyan mb-4" />
...
const VestingScheduleDetails = ({ schedule }: { schedule: VestingSchedule }) => {
    ...
    return (
        <div className="glass-card p-8 space-y-6 card-3d-hover">
            ...
            <div className="grid md:grid-cols-2 gap-x-8 gap-y-4 border-t border-dark-border pt-6">
            ...
            <div className="bg-dark-bg/50 p-6 rounded-lg text-center border border-dark-border">
                <p className="text-lg font-semibold text-text-secondary">{t('claimable_now')}</p>
                <p className="text-4xl font-bold my-2 text-neon-cyan" style={{ textShadow: '0 0 10px var(--neon-cyan)'}}>{claimableNow.toLocaleString(undefined, {maximumFractionDigits: 2})}</p>
                <button onClick={handleClaim} disabled={solana.loading || claimableNow <= 0} className="mt-4 w-full max-w-xs neon-button font-bold py-3 rounded-full text-lg disabled:opacity-50">
...
const AdminPortal = () => {
    ...
    return (
        <div className="mt-12 space-y-8">
            <div className="glass-card p-8 card-3d-hover">
                <h2 className="text-2xl font-bold mb-6 flex items-center"><PlusCircle className="mr-3 text-neon-cyan"/>{t('create_vesting_schedule')}</h2>
                <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                    <input type="text" placeholder={t('recipient_address')} value={recipient} onChange={e => setRecipient(e.target.value)} required className="w-full p-3 bg-dark-card border border-dark-border rounded-md" />
                    <input type="number" placeholder={t('total_amount_owfn')} value={amount} onChange={e => setAmount(e.target.value)} required className="w-full p-3 bg-dark-card border border-dark-border rounded-md" />
                    <input type="number" placeholder={t('vesting_duration_months')} value={duration} onChange={e => setDuration(e.target.value)} required className="w-full p-3 bg-dark-card border border-dark-border rounded-md" />
                    <input type="number" placeholder={t('cliff_period_months')} value={cliff} onChange={e => setCliff(e.target.value)} required className="w-full p-3 bg-dark-card border border-dark-border rounded-md" />
                    <button type="submit" className="md:col-span-2 w-full neon-button-magenta py-3 rounded-full font-bold">{t('create_schedule')}</button>
                </form>
            </div>
            <div className="glass-card p-8 card-3d-hover">
                 ...
                 <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                     {vestingSchedules.map(schedule => (
                         <div key={schedule.recipientAddress} className="bg-dark-card p-4 rounded-lg border border-dark-border">
...
export default function Vesting() {
    const { t, solana, vestingSchedules, isVerified } = useAppContext();
    ...
    return (
        <div className="animate-fade-in-up space-y-12">
            <div className="text-center">
                <h1 className="text-4xl font-bold text-text-primary" style={{ textShadow: '0 0 10px var(--neon-cyan)' }}>{t('vesting_title')}</h1>
                <p className="mt-4 max-w-2xl mx-auto text-lg text-text-secondary">
                    {t('vesting_subtitle')}
                </p>
            </div>
            
            {!solana.connected || !isVerified ? <ConnectWalletPrompt /> : (
                <>
                    {userSchedule && <VestingScheduleDetails schedule={userSchedule} />}
                    {!userSchedule && !isAdmin && <NoScheduleInfo />}
                    {isAdmin && <AdminPortal />}
                </>
            )}
        </div>
