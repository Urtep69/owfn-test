  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="relative container mx-auto px-4">
        <div className="absolute h-full border-l-4 border-primary-200 dark:border-darkAccent-700/50 left-1/2 -translate-x-1/2"></div>
        {ROADMAP_DATA.map((phase, index) => (
          <div key={phase.quarter} className={`mb-8 flex justify-between items-center w-full ${index % 2 === 0 ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="z-20 flex items-center order-1 bg-darkAccent-500 shadow-xl w-12 h-12 rounded-full">
              <h1 className="mx-auto font-semibold text-lg text-darkPrimary-950">{index + 1}</h1>
            </div>
            <div className={`order-1 bg-white dark:bg-darkPrimary-800 rounded-lg shadow-3d w-5/12 px-6 py-4 transition-transform transform hover:scale-105 golden-border ${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
