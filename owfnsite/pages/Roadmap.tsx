  ...
  return (
    <div className="animate-fade-in-up">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-display font-bold text-accent">{t('roadmap_title')}</h1>
      ...
      <div className="relative container mx-auto px-4">
        ...
        {ROADMAP_DATA.map((phase, index) => (
          <div key={phase.quarter} className={`mb-8 flex justify-between items-center w-full ${index % 2 === 0 ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="z-20 flex items-center order-1 bg-accent shadow-glow-accent w-12 h-12 rounded-full">
              <h1 className="mx-auto font-semibold text-lg text-accent-foreground">{index + 1}</h1>
            </div>
            <div className={`order-1 glassmorphism-dark rounded-lg w-5/12 px-6 py-4 transition-transform transform hover:scale-105 ${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
              <p className="text-accent font-semibold">{phase.quarter}</p>
