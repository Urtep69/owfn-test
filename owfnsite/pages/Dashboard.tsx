    ...
    return (
        <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
            ...
            {loading ? (
                ...
                <>
                    <div className="grid grid-cols-2 gap-4 mb-4 p-4 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                        <div>
                            <p className="text-sm text-slate-500 dark:text-slate-400">{t('token_types')}</p>
                            <p className="text-2xl font-bold">{balances.length}</p>
                        </div>
                        <div className="text-right">
                             <p className="text-sm text-slate-500 dark:text-slate-400">{t('total_value')}</p>
                    ...
                    <div className="space-y-1 max-h-60 overflow-y-auto pr-2">
                        {balances.length > 0 ? balances.map(token => (
                             <Link key={token.mintAddress} to={`/dashboard/token/${token.mintAddress}?from=/dashboard`}>
                                <a className="grid grid-cols-2 gap-4 items-center py-2 px-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors cursor-pointer">
                                    {/* Asset Info */}
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-slate-400">${token.usdValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                    </div>
                                </a>
                            </Link>
                        )) : (
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="text-center">
                <h1 className="text-4xl font-bold text-primary-600 dark:text-primary-400">{t('wallet_monitor')}</h1>
