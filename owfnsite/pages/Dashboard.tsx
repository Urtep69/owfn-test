    ...
    return (
        <div className="glass-card p-6">
            ...
            {loading ? (
                ...
                <>
                    <div className="grid grid-cols-2 gap-4 mb-4 p-4 bg-dark-card/50 rounded-lg">
                        <div>
                    ...
                    <div className="space-y-1 max-h-60 overflow-y-auto pr-2">
                        {balances.length > 0 ? balances.map(token => (
                             <Link key={token.mintAddress} to={`/dashboard/token/${token.mintAddress}?from=/dashboard`}>
                                <a className="grid grid-cols-2 gap-4 items-center py-2 px-2 rounded-md hover:bg-dark-card transition-colors cursor-pointer">
                                    {/* Asset Info */}
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="text-center">
                <h1 className="text-4xl font-bold text-text-primary" style={{ textShadow: '0 0 10px var(--neon-cyan)' }}>{t('wallet_monitor')}</h1>
