...
export default function ImpactCaseDetail() {
    ...
    if (!socialCase) {
        return (
            <div className="text-center py-10 animate-fade-in-up">
                ...
                <Link to="/impact" className="text-neon-cyan hover:underline mt-4 inline-block">{t('back_to_all_cases')}</Link>
            </div>
        );
    }
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <Link to={`/impact/category/${categorySlug}`} className="inline-flex items-center gap-2 text-neon-cyan hover:underline">
            ...
            <div className="glass-card overflow-hidden">
                ...
                <div className="p-6 md:p-10">
                    <span className="text-lg font-semibold text-neon-magenta mb-2 inline-block">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                    ...
                    <div className="mb-8">
                        ...
                        <div className="flex justify-between text-lg font-semibold mt-2">
                            <span><span className="font-bold text-neon-cyan">${socialCase.donated.toLocaleString()}</span> {t('funded')}</span>
                     ...
                     <div className="glass-card p-6">
                        ...
                        <div className="space-y-4">
                            {mockUpdates.map(update => (
                                <div key={update.date} className="border-b border-dark-border pb-4 last:border-b-0 last:pb-0">
                ...
                <div className="lg:col-span-2">
                    <div className="lg:sticky top-24 space-y-8">
                        <div className="glass-card p-6">
                            ...
                            <div className="bg-neon-magenta/10 border border-neon-magenta/30 text-neon-magenta/80 p-3 rounded-md text-sm flex items-start space-x-2 mb-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="0.0"
                                        className="w-full p-3 bg-dark-card border border-dark-border rounded-lg text-lg font-semibold focus:ring-2 focus:ring-neon-cyan focus:outline-none"
                                    />
                        ...
                        <div className="glass-card p-6">
                            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Milestone /> {t('funding_milestones')}</h3>
                             <div className="relative pl-4">
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-dark-border"></div>
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-neon-cyan transition-all duration-500" style={{ height: `${progress}%` }}></div>
                                {milestones.map(milestone => (
                                    <div key={milestone.percentage} className="flex items-start mb-4 relative">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center -ml-8 flex-shrink-0 z-10 ${progress >= milestone.percentage ? 'bg-neon-cyan' : 'bg-dark-border'}`}>
                                            <CheckCircle size={16} className="text-dark-bg"/>
                                        </div>
                                        <div className="ml-4">
