...
export default function ImpactCaseDetail() {
    ...
    if (!socialCase) {
        return (
            <div className="text-center py-10 animate-fade-in-up">
                ...
                <Link to="/impact" className="text-primary hover:underline mt-4 inline-block">{t('back_to_all_cases')}</Link>
            </div>
        );
    }
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <Link to={`/impact/category/${categorySlug}`} className="inline-flex items-center gap-2 text-primary hover:underline">
            ...
            <div className="bg-surface border border-border rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="p-6 md:p-10">
                    <span className="text-lg font-semibold text-primary mb-2 inline-block">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                    <h1 className="text-3xl md:text-5xl font-bold mb-6">{title}</h1>
                    <p className="text-lg text-foreground-muted leading-relaxed mb-8">{description}</p>
                    
                    <div className="mb-8">
                        <ProgressBar progress={progress} />
                        <div className="flex justify-between text-lg font-semibold mt-2">
                            <span><span className="font-bold text-primary">${socialCase.donated.toLocaleString()}</span> {t('funded')}</span>
                            <span><span className="font-normal text-foreground-muted">{t('goal')}:</span> ${socialCase.goal.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-3 space-y-8">
                     <div className="bg-surface border border-border p-6 rounded-lg shadow-lg">
                        ...
                        <div className="space-y-4">
                            {mockUpdates.map(update => (
                                <div key={update.date} className="border-b border-border pb-4 last:border-b-0 last:pb-0">
                                    <p className="text-sm text-foreground-muted font-semibold">{update.date}</p>
                                    <p className="text-foreground mt-1">{t(update.key)}</p>
                     ...
                     {details && (
                        <div className="bg-surface border border-border p-6 rounded-lg shadow-lg">
                            ...
                            <p className="text-foreground-muted leading-relaxed whitespace-pre-wrap">{details}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="lg:sticky top-24 space-y-8">
                        <div className="bg-surface border border-border p-6 rounded-lg shadow-lg">
                            ...
                            <div className="bg-secondary/10 border border-secondary/30 text-secondary p-3 rounded-md text-sm flex items-start space-x-2 mb-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="0.0"
                                        className="w-full p-3 bg-background border border-border rounded-lg text-lg font-semibold focus:ring-2 focus:ring-ring focus:outline-none"
                                    />
                        ...
                        <div className="bg-surface border border-border p-6 rounded-lg shadow-lg">
                            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Milestone /> {t('funding_milestones')}</h3>
                             <div className="relative pl-4">
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-border"></div>
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-primary transition-all duration-500" style={{ height: `${progress}%` }}></div>
                                {milestones.map(milestone => (
                                    <div key={milestone.percentage} className="flex items-start mb-4 relative">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center -ml-8 flex-shrink-0 z-10 ${progress >= milestone.percentage ? 'bg-primary' : 'bg-border'}`}>
                                            <CheckCircle size={16} className="text-white"/>
                                        </div>
                                        <div className="ml-4">
                                            <p className="font-bold">{milestone.percentage}%</p>
                                            <p className="text-sm text-foreground-muted">{t(milestone.key)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
