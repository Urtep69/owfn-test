...
export default function ImpactCaseDetail() {
    ...
    if (!socialCase) {
        return (
            <div className="text-center py-10 animate-fade-in-up">
                ...
                <Link to="/impact" className="text-primary-500 dark:text-primary-500 hover:underline mt-4 inline-block">{t('back_to_all_cases')}</Link>
            </div>
        );
    }
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <Link to={`/impact/category/${categorySlug}`} className="inline-flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:underline">
                <ArrowLeft size={16} /> {t('back_to_category_cases', { category: categoryName })}
            </Link>
            <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
                <img src={socialCase.imageUrl} alt={title} className="w-full h-64 md:h-96 object-cover" />
                <div className="p-6 md:p-10">
                    <span className="text-lg font-semibold text-primary-600 dark:text-primary-500 mb-2 inline-block">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                    <h1 className="text-3xl md:text-5xl font-bold mb-6">{title}</h1>
                    <p className="text-lg text-slate-700 dark:text-slate-300 leading-relaxed mb-8">{description}</p>
                    
                    <div className="mb-8">
                        <ProgressBar progress={progress} />
                        <div className="flex justify-between text-lg font-semibold mt-2">
                            <span><span className="font-bold text-primary-600 dark:text-primary-400">${socialCase.donated.toLocaleString()}</span> {t('funded')}</span>
                            <span><span className="font-normal text-slate-600 dark:text-slate-400">{t('goal')}:</span> ${socialCase.goal.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-3 space-y-8">
                     <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                        ...
                        <div className="space-y-4">
                            {mockUpdates.map(update => (
                                <div key={update.date} className="border-b border-slate-200 dark:border-slate-700 pb-4 last:border-b-0 last:pb-0">
                                    <p className="text-sm text-slate-500 dark:text-slate-400 font-semibold">{update.date}</p>
                                    <p className="text-slate-700 dark:text-slate-300 mt-1">{t(update.key)}</p>
                     ...
                     {details && (
                        <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                            ...
                            <p className="text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{details}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="lg:sticky top-24 space-y-8">
                        <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                            ...
                            <div className="bg-yellow-500/10 border border-yellow-500/20 text-yellow-800 dark:text-yellow-200 p-3 rounded-md text-sm flex items-start space-x-2 mb-6">
                                <AlertTriangle className="h-5 w-5 flex-shrink-0 mt-0.5" />
                                <p>{t('donation_solana_warning')}</p>
                            </div>
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">{t('select_token')}</label>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                        {tokens.map(token => (
                                            <button
                                                key={token.symbol}
                                                onClick={() => setSelectedToken(token.symbol)}
                                                className={`flex flex-col items-center justify-center p-4 border-2 rounded-lg transition-all ${selectedToken === token.symbol ? 'border-primary-500 bg-primary-500/10' : 'border-slate-200 dark:border-slate-600'}`}
                                            >
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="0.0"
                                        className="w-full p-3 bg-slate-100 dark:bg-slate-700 rounded-lg text-lg font-semibold focus:ring-2 focus:ring-primary-500 focus:outline-none"
                                    />
                                    <div className="flex flex-wrap gap-2 mt-3">
                                        {percentages.map(p => (
                                            <button
                                                ...
                                                className="flex-grow text-xs bg-slate-200/50 hover:bg-slate-200 dark:bg-slate-700/50 dark:hover:bg-slate-700 py-1 px-3 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                                                {p === 100 ? 'MAX' : `${p}%`}
                                            </button>
                                        ))}
                                ...
                                {solana.connected && (
                                    <div className="animate-fade-in-up" style={{animationDuration: '300ms'}}>
                                        {currentUserToken ? (
                                            ...
                                            <div className="text-center py-3 px-4 bg-primary-500/10 border border-primary-500/20 rounded-lg">
                                ...
                                {parseFloat(amount) > 0 && (
                                    <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-lg text-center animate-fade-in-up" style={{animationDuration: '300ms'}}>
                                        <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                                            {parseFloat(amount).toLocaleString(undefined, {maximumFractionDigits: 4})} {selectedToken}
                                        </p>
                                        <p className="text-md text-slate-700 dark:text-slate-300 font-semibold">
                        ...
                        <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                            <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Milestone /> {t('funding_milestones')}</h3>
                             <div className="relative pl-4">
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-slate-200 dark:bg-slate-700"></div>
                                <div className="absolute top-0 left-4 h-full w-0.5 bg-primary-500 transition-all duration-500" style={{ height: `${progress}%` }}></div>
                                {milestones.map(milestone => (
                                    <div key={milestone.percentage} className="flex items-start mb-4 relative">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center -ml-8 flex-shrink-0 z-10 ${progress >= milestone.percentage ? 'bg-primary-500' : 'bg-slate-300 dark:bg-slate-600'}`}>
                                            <CheckCircle size={16} className="text-white"/>
                                        </div>
                                        <div className="ml-4">
                                            <p className="font-bold">{milestone.percentage}%</p>
                                            <p className="text-sm text-slate-600 dark:text-slate-400">{t(milestone.key)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
