...
const StatCard = ({ title, value, icon }: { title: string, value: string | number, icon: React.ReactNode }) => (
    <div className="glassmorphism p-6 rounded-lg flex items-center space-x-4">
        <div className="bg-surface-2 text-accent rounded-full p-3">
            {icon}
        </div>
        <div>
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="flex justify-between items-center">
                ...
                <button onClick={fetchAllTransactions} disabled={loading} className="flex items-center gap-2 bg-surface-2 px-4 py-2 rounded-lg font-semibold hover:bg-surface-3 disabled:opacity-50">
            ...
            {loading ? (
                <div className="flex justify-center items-center py-20"><Loader2 className="w-12 h-12 animate-spin text-accent"/></div>
            ) : (
                <>
                    ...
                    <div className="glassmorphism p-6 rounded-lg">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={exportToCsv} className="flex items-center gap-2 bg-surface-2 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-surface-3">
                                <Download size={16} /> {t('export_csv')}
                            </button>
                        </div>
                        <div className="overflow-x-auto max-h-[50vh] relative">
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-text-secondary uppercase bg-surface-2 sticky top-0">
                                    <tr>
                                ...
                                <tbody>
                                    {transactions.map(tx => (
                                        <tr key={tx.signature} className="border-b border-border-color hover:bg-surface-2">
                    ...
                    <div className="glassmorphism p-6 rounded-lg space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={handleClearProgress} disabled={isAirdropping} className="flex items-center gap-2 text-xs bg-danger/20 text-danger px-3 py-1.5 rounded-lg font-semibold hover:bg-danger/30 disabled:opacity-50">
                                <RotateCcw size={14} /> Clear Progress & Logs
                            </button>
                        </div>
                         <div className="bg-danger/10 border-l-4 border-danger text-danger p-4 rounded-r-lg flex items-start gap-3">
                         ...
                         <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-surface-2 p-4 rounded-lg">
                             ...
                             <div className={`bg-surface-2 p-4 rounded-lg ${adminOwfnBalance < totalOwfnToDistribute ? 'border border-danger' : ''}`}>
                                ...
                                {adminOwfnBalance < totalOwfnToDistribute && <p className="text-xs text-danger mt-1">{t('insufficient_owfn_balance')}</p>}
                            </div>
                             <div className={`bg-surface-2 p-4 rounded-lg ${adminSolBalance < estimatedFees.total ? 'border border-danger' : ''}`}>
                                ...
                                {adminSolBalance < estimatedFees.total && <p className="text-xs text-danger mt-1">{t('insufficient_sol_balance')}</p>}
                            </div>
                         </div>
                         ...
                         <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-accent text-accent-foreground font-bold py-3 px-6 rounded-lg hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed"
                         >
                         ...
                         {(isAirdropping || airdropProgress.total > 0) && (
                             <div>
                                 ...
                                 <div className="w-full bg-surface-2 rounded-full h-4">
                                    <div className="bg-success h-4 rounded-full transition-all duration-300" style={{width: `${(airdropProgress.current / (airdropProgress.total || 1)) * 100}%`}}></div>
                                 </div>
                             </div>
                         )}
                                ...
                                <div className="space-y-1 text-xs font-mono">
                                    {airdropLogs.map((log, i) => (
                                        <div key={i} className="flex items-center gap-2">
                                            {log.startsWith('✅') ? <CheckCircle className="w-3 h-3 text-success flex-shrink-0"/> : (log.startsWith('❌') ? <XCircle className="w-3 h-3 text-danger flex-shrink-0"/> : <div className="w-3 h-3 flex-shrink-0"></div>)}
