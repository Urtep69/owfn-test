...
const StatCard = ({ title, value, icon }: { title: string, value: string | number, icon: React.ReactNode }) => (
    <div className="bg-surface border border-border p-6 rounded-lg flex items-center space-x-4">
        <div className="bg-background text-primary rounded-full p-3">
            {icon}
        </div>
        <div>
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="flex justify-between items-center">
                ...
                <button onClick={fetchAllTransactions} disabled={loading} className="flex items-center gap-2 bg-surface border border-border px-4 py-2 rounded-lg font-semibold hover:bg-border disabled:opacity-50">
                ...
                <>
                    ...
                    <div className="bg-surface border border-border p-6 rounded-lg">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={exportToCsv} className="flex items-center gap-2 bg-background border border-border px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-border">
                        ...
                        <div className="overflow-x-auto max-h-[50vh] relative">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {transactions.map(tx => (
                                        <tr key={tx.signature} className="border-b border-border hover:bg-background">
                    ...
                    <div className="bg-surface border border-border p-6 rounded-lg space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={handleClearProgress} disabled={isAirdropping} className="flex items-center gap-2 text-xs bg-destructive/10 text-destructive px-3 py-1.5 rounded-lg font-semibold hover:bg-destructive/20 disabled:opacity-50">
                                <RotateCcw size={14} /> Clear Progress & Logs
                            </button>
                        </div>
                         <div className="bg-destructive/10 border-l-4 border-destructive text-destructive p-4 rounded-r-lg flex items-start gap-3">
                         ...
                         <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-background border border-border p-4 rounded-lg">
                             ...
                             <div className={`bg-background border border-border p-4 rounded-lg ${adminOwfnBalance < totalOwfnToDistribute ? 'border-destructive' : ''}`}>
                                ...
                                {adminOwfnBalance < totalOwfnToDistribute && <p className="text-xs text-destructive mt-1">{t('insufficient_owfn_balance')}</p>}
                            </div>
                             <div className={`bg-background border border-border p-4 rounded-lg ${adminSolBalance < estimatedFees.total ? 'border-destructive' : ''}`}>
                                ...
                                {adminSolBalance < estimatedFees.total && <p className="text-xs text-destructive mt-1">{t('insufficient_sol_balance')}</p>}
                            </div>
                         </div>
                         ...
                         <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-primary text-white font-bold py-3 px-6 rounded-lg hover:bg-opacity-80 disabled:opacity-50 disabled:cursor-not-allowed"
                         >
                         ...
                         {(isAirdropping || airdropProgress.total > 0) && (
                             <div>
                                 ...
                                 <div className="w-full bg-border rounded-full h-4">
                                    <div className="bg-secondary h-4 rounded-full transition-all duration-300" style={{width: `${(airdropProgress.current / (airdropProgress.total || 1)) * 100}%`}}></div>
                                 </div>
                             </div>
                         )}

                         {airdropLogs.length > 0 && (
                             <div className="bg-background border border-border p-4 rounded-lg max-h-64 overflow-y-auto">
                                <h3 className="font-bold mb-2">{t('airdrop_log')}</h3>
                                <div className="space-y-1 text-xs font-mono">
                                    {airdropLogs.map((log, i) => (
                                        <div key={i} className="flex items-center gap-2">
                                            {log.startsWith('✅') ? <CheckCircle className="w-3 h-3 text-secondary flex-shrink-0"/> : (log.startsWith('❌') ? <XCircle className="w-3 h-3 text-destructive flex-shrink-0"/> : <div className="w-3 h-3 flex-shrink-0"></div>)}
