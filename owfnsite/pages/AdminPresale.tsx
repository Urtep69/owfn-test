...
const LOCAL_STORAGE_KEY = 'owfn-airdrop-processed';
...
export default function AdminPresale() {
    ...
    const aggregatedContributors = useMemo<AggregatedContributor[]>(() => {
        ...
        const individualContributions = new Map<string, bigint>();

        transactions.forEach(tx => {
            const currentTotal = individualContributions.get(tx.from) || 0n;
            individualContributions.set(tx.from, currentTotal + BigInt(tx.lamports));
        });

        individualContributions.forEach((totalLamports, address) => {
            let totalOwfn = (totalLamports * presaleRateBigInt * owfnDecimalsMultiplier) / BigInt(LAMPORTS_PER_SOL);
            if (totalLamports >= bonusThresholdLamports) {
                const bonusAmount = (totalOwfn * BigInt(PRESALE_DETAILS.bonusPercentage)) / 100n;
                totalOwfn += bonusAmount;
            }
            contributorMap.set(address, { totalLamports, totalOwfn });
        });
    ...
    useEffect(() => {
        setAirdropProgress(prev => ({ ...prev, current: processedAddresses.size, total: aggregatedContributors.length }));
    }, [processedAddresses, aggregatedContributors]);

    const exportToCsv = useCallback(() => {
        const headers = ['contributor_address', 'total_sol_spent', 'total_owfn_to_receive_with_bonus'];
    ...
    const updateProcessedAddresses = (newlyProcessed: string[]) => {
        const newSet = new Set([...processedAddresses, ...newlyProcessed]);
        setProcessedAddresses(newSet);
        try {
            localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(Array.from(newSet)));
        } catch (e) {
            console.error("Failed to save airdrop progress to localStorage", e);
        }
    };
    ...
    const handleAirdrop = useCallback(async () => {
        ...
        if (contributorsToProcess.length === 0 && aggregatedContributors.length > 0) {
            toast.info("All contributors have been processed.");
            return;
        }
        ...
        if (processedAddresses.size === 0) setAirdropLogs([]);
        ...
        let newlyProcessedInRun: string[] = [];
            ...
            try {
                ...
                for (let j = 0; j < signedTxs.length; j++) {
                    ...
                    try {
                        ...
                        newlyProcessedInRun.push(contributor.address);
        ...
        updateProcessedAddresses(newlyProcessedInRun);
        setIsAirdropping(false);
        const totalFailed = contributorsToProcess.length - successfulInThisRun;
        const logMessage = `--- Airdrop run complete. Success: ${successfulInThisRun}, Failed: ${totalFailed}. Total processed: ${processedAddresses.size + newlyProcessedInRun.length}/${aggregatedContributors.length} ---`;
        setAirdropLogs(prev => [...prev, logMessage]);
        toast.success("Airdrop run finished.", { description: logMessage });
    ...
    const handleClearProgress = () => {
        ...
        if (window.confirm("Are you sure you want to clear the airdrop progress and logs? This will reset the counter to zero. This action cannot be undone and is intended for emergency resets.")) {
            setProcessedAddresses(new Set());
            setAirdropLogs([]);
            setAirdropProgress({ current: 0, total: 0 });
            localStorage.removeItem(LOCAL_STORAGE_KEY);
            toast.success("Airdrop progress has been reset.");
        }
    };
