...
const StatCard = ({ title, value, icon }: { title: string, value: string | number, icon: React.ReactNode }) => (
    <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg flex items-center space-x-4">
        <div className="bg-slate-100 dark:bg-slate-700 text-primary-500 dark:text-primary-400 rounded-full p-3">
            {icon}
        </div>
        <div>
        ...
        <div className="animate-fade-in-up space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 mt-1">{t('presale_admin_subtitle')}</p>
                </div>
                <button onClick={fetchAllTransactions} disabled={loading} className="flex items-center gap-2 bg-slate-200 dark:bg-slate-800 px-4 py-2 rounded-lg font-semibold hover:bg-slate-300 dark:hover:bg-slate-700 disabled:opacity-50">
            ...
            {loading ? (
                <div className="flex justify-center items-center py-20"><Loader2 className="w-12 h-12 animate-spin text-primary-500"/></div>
            ) : (
                <>
                    ...
                    <div className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-lg">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={exportToCsv} className="flex items-center gap-2 bg-slate-200 dark:bg-slate-800 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-slate-300 dark:hover:bg-slate-700">
                        ...
                        <div className="overflow-x-auto max-h-[50vh] relative">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {transactions.map(tx => (
                                        <tr key={tx.signature} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                        ...
                        <div className="flex justify-between items-center">
                         ...
                         <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg">
                                <p className="text-sm text-slate-600 dark:text-slate-400">{t('total_owfn_to_distribute')}</p>
                                <p className="text-lg font-bold font-mono">{totalOwfnToDistribute.toLocaleString(undefined, {maximumFractionDigits: 2})}</p>
                            </div>
                             <div className={`bg-slate-100 dark:bg-slate-700 p-4 rounded-lg ${adminOwfnBalance < totalOwfnToDistribute ? 'border border-red-500' : ''}`}>
                                <p className="text-sm text-slate-600 dark:text-slate-400">{t('your_owfn_balance')}</p>
                             ...
                             <div className={`bg-slate-100 dark:bg-slate-700 p-4 rounded-lg ${adminSolBalance < estimatedFees.total ? 'border border-red-500' : ''}`}>
                                <p className="text-sm text-slate-600 dark:text-slate-400">{t('your_sol_balance')}</p>
                         ...
                         <button 
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-primary-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
                         >
                         ...
                         {airdropLogs.length > 0 && (
                             <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg max-h-64 overflow-y-auto">
