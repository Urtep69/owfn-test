...
const Section = ({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) => (
    <section className="bg-surface border border-border p-8 rounded-lg mb-12">
        ...
        <div className="prose prose-lg prose-p:text-text-secondary prose-h3:text-text-primary max-w-none leading-relaxed">
            {children}
        </div>
    </section>
);
                    ...
                    <div className="space-y-3">
                        {TOKEN_ALLOCATIONS.map(alloc => (
                            <div key={alloc.name} className="flex items-center space-x-3">
                                ...
                                <span className="text-base text-text-secondary">{alloc.name} - <strong>{alloc.percentage}%</strong></span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="relative border-l-2 border-border ml-4 pl-8 space-y-8">
                    {ROADMAP_DATA.map((phase) => (
                        <div key={phase.quarter} className="relative">
                            ...
                            <h4 className="font-bold text-xl text-text-primary">{t(`${phase.key_prefix}_title`)}</h4>
