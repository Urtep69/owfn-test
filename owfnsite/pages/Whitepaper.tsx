...
const Section = ({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) => (
    <section className="bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-lg mb-12">
        <div className="flex items-center mb-6">
            <div className="bg-slate-100 dark:bg-slate-700 text-primary-500 dark:text-primary-400 rounded-full p-3 mr-4">
                {icon}
            </div>
...
const DetailItem = ({ label, value }: { label: string, value: React.ReactNode }) => (
    <div className="flex flex-col sm:flex-row justify-between py-3 border-b border-slate-200 dark:border-slate-700">
        <span className="text-slate-600 dark:text-slate-400 font-medium">{label}</span>
        <span className="font-semibold text-slate-800 dark:text-slate-100 text-left sm:text-right">{value}</span>
    </div>
);
    ...
    return (
        <div className="animate-fade-in-up">
            <header className="text-center mb-16">
                <FileText className="mx-auto w-20 h-20 text-primary-500 mb-4" />
                <h1 className="text-5xl font-extrabold text-primary-600 dark:text-primary-400">{t('whitepaper_title')}</h1>
            ...
            <Section title={t('roadmap_title')} icon={<MapIcon />}>
                <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-4 pl-8 space-y-8">
                    {ROADMAP_DATA.map((phase) => (
                        <div key={phase.quarter} className="relative">
                             <div className="absolute -left-[42px] top-1 w-6 h-6 bg-primary-400 rounded-full border-4 border-slate-50 dark:border-slate-800"></div>
                            <p className="font-semibold text-primary-500 dark:text-primary-400">{phase.quarter}</p>
            ...
            <Section title={t('whitepaper_community_title')} icon={<LinkIcon />}>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                    <a href={PROJECT_LINKS.website} target="_blank" rel="noopener noreferrer" className="text-primary-600 dark:text-primary-400 hover:underline font-semibold">Website</a>
                    <a href={PROJECT_LINKS.x} target="_blank" rel="noopener noreferrer" className="text-primary-600 dark:text-primary-400 hover:underline font-semibold">X.com (Twitter)</a>
                    <a href={PROJECT_LINKS.telegramGroup} target="_blank" rel="noopener noreferrer" className="text-primary-600 dark:text-primary-400 hover:underline font-semibold">Telegram Group</a>
                    <a href={PROJECT_LINKS.discord} target="_blank" rel="noopener noreferrer" className="text-primary-600 dark:text-primary-400 hover:underline font-semibold">Discord</a>
                </div>
            </Section>
        </div>
    );
