...
const Section = ({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) => (
    <section className="glass-card p-8 mb-12">
        <div className="flex items-center mb-6">
            <div className="bg-dark-card text-neon-cyan rounded-full p-3 mr-4">
                {icon}
            </div>
            <h2 className="text-3xl font-bold text-text-primary">{title}</h2>
        </div>
        <div className="prose prose-lg prose-invert max-w-none text-text-secondary leading-relaxed">
            {children}
        </div>
    </section>
);
...
const DetailItem = ({ label, value }: { label: string, value: React.ReactNode }) => (
    <div className="flex flex-col sm:flex-row justify-between py-3 border-b border-dark-border">
    ...
    return (
        <div className="animate-fade-in-up">
            <header className="text-center mb-16">
                <FileText className="mx-auto w-20 h-20 text-neon-cyan mb-4" />
                <h1 className="text-5xl font-extrabold text-text-primary" style={{ textShadow: '0 0 10px var(--neon-cyan)' }}>{t('whitepaper_title')}</h1>
                ...
                <div className="relative border-l-2 border-dark-border ml-4 pl-8 space-y-8">
                    {ROADMAP_DATA.map((phase) => (
                        <div key={phase.quarter} className="relative">
                             <div className="absolute -left-[42px] top-1 w-6 h-6 bg-neon-cyan rounded-full border-4 border-dark-card"></div>
                            <p className="font-semibold text-neon-magenta">{phase.quarter}</p>
            ...
            <Section title={t('whitepaper_community_title')} icon={<LinkIcon />}>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                    <a href={PROJECT_LINKS.website} target="_blank" rel="noopener noreferrer" className="text-neon-cyan hover:underline font-semibold">Website</a>
                    <a href={PROJECT_LINKS.x} target="_blank" rel="noopener noreferrer" className="text-neon-cyan hover:underline font-semibold">X.com (Twitter)</a>
                    <a href={PROJECT_LINKS.telegramGroup} target="_blank" rel="noopener noreferrer" className="text-neon-cyan hover:underline font-semibold">Telegram Group</a>
                    <a href={PROJECT_LINKS.discord} target="_blank" rel="noopener noreferrer" className="text-neon-cyan hover:underline font-semibold">Discord</a>
                </div>
            </Section>
        </div>
    );
