export const pt = {
    ...
    faq_a1: 'OWFN (Official World Family Network) é um token baseado em Solana projetado para unir famílias globalmente através da tecnologia blockchain, focando em impacto social, educação, saúde e ajuda humanitária com total transparência.',
    ...
    faq_a5: 'OWFN é um Token que Rende Juros (IBT) que recompensa automaticamente seus detentores com um Rendimento Percentual Anual (APY) de 2%. Isso significa que suas participações em OWFN crescerão ao longo do tempo simplesmente mantendo-as em sua carteira.',
    ...
    faq_a10: 'Sim, isso é de importância crítica. Todas as doações de USDC e USDT devem ser enviadas *exclusivamente* da blockchain Solana. Não envie de nenhuma outra rede (ex: Ethereum), pois os fundos serão perdidos e não chegarão ao seu destino. Esta informação também é exibida claramente na página de doações.',
    ...
    airdrop_summary: '{success} bem-sucedidas, {failed} falhas.',
