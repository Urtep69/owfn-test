...
export const en = {
    ...
    faq_search_aria_label: `Search through frequently asked questions`,
    ...
    faq_q11: `What happens exactly after I buy tokens in the presale? Do I need to do anything?`,
    faq_a11: `No, after you have successfully completed the purchase of OWFN tokens during the presale, no further action is required on your part. The tokens are automatically reserved for your wallet address. At the end of the presale period, all tokens will be automatically distributed (via airdrop) to the buyers' wallets.`,
    faq_q12: `When and where will I be able to trade (sell/buy) OWFN tokens after the presale?`,
    faq_a12: `According to our Roadmap, after the successful conclusion of the presale, we will proceed with the official listing of the $OWFN token on decentralized exchanges (DEX) within the Solana ecosystem. The exact date and platforms will be announced on our official channels.`,
    faq_q13: `Why is there a maximum buy limit (Max Buy) in the presale?`,
    faq_a13: `The maximum buy limit is an important measure to ensure the fairest possible distribution of tokens and to prevent a small number of large investors ("whales") from controlling a significant portion of the supply. This promotes a healthier and more decentralized community in the long run.`,
    faq_q14: `What does it mean that the token is "Interest-Bearing" with 2% APY? How does it work?`,
    faq_a14: `It means the OWFN token is programmed to automatically generate interest. Just by holding the tokens in your Solana-compatible wallet, the quantity of your tokens will increase over time, with a 2% Annual Percentage Yield (APY). You do not need to stake them or take any additional action; the rewards accumulate automatically.`,
    ...
    faq_a20: `Security is our top priority. We use multi-signature wallets for managing critical funds, which means no transaction can be approved by a single person. The token's smart contract will be audited by reputable security firms to identify and correct any potential vulnerabilities before launch.`,
    ...
    contact_disclaimer_message: `To ensure our team can effectively support the community and advance the project, we prioritize inquiries directly related to the Official World Family Network. Messages that are off-topic, spam, or unsolicited may not receive a response. Thank you for your understanding and for helping us stay focused on our mission.`,
    ...
    contact_social_discord_desc: `Become a part of our active community for discussions and support.`,
};