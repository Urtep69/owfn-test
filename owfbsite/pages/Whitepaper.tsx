...
const Section = ({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) => (
    <section className="bg-primary-800 p-8 rounded-xl shadow-3d-lg mb-12">
        <div className="flex items-center mb-6">
            <div className="bg-primary-700 text-accent-400 rounded-full p-3 mr-4">
                {icon}
            </div>
            <h2 className="text-3xl font-bold text-primary-100">{title}</h2>
        </div>
        <div className="prose prose-lg dark:prose-invert max-w-none text-primary-300 leading-relaxed">
            {children}
        </div>
    </section>
);
...
const DetailItem = ({ label, value }: { label: string, value: React.ReactNode }) => (
    <div className="flex flex-col sm:flex-row justify-between py-3 border-b border-primary-700">
        <span className="text-primary-400 font-medium">{label}</span>
        <span className="font-semibold text-primary-100 text-left sm:text-right">{value}</span>
    </div>
);
    ...
    return (
        <div className="animate-fade-in-up">
            <header className="text-center mb-16">
                ...
                <h1 className="text-5xl font-extrabold text-accent-400">{t('whitepaper_title')}</h1>
                <p className="mt-4 text-xl text-primary-400 max-w-3xl mx-auto">
            ...
            <Section title={t('roadmap_title')} icon={<MapIcon />}>
                <div className="relative border-l-2 border-primary-700 ml-4 pl-8 space-y-8">
                    {ROADMAP_DATA.map((phase) => (
                        <div key={phase.quarter} className="relative">
                             <div className="absolute -left-[42px] top-1 w-6 h-6 bg-accent-500 rounded-full border-4 border-primary-800"></div>
                            <p className="font-semibold text-accent-400">{phase.quarter}</p>
            ...
            <Section title={t('whitepaper_community_title')} icon={<LinkIcon />}>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                    <a href={PROJECT_LINKS.website} target="_blank" rel="noopener noreferrer" className="text-accent-400 hover:underline font-semibold">Website</a>
                    <a href={PROJECT_LINKS.x} target="_blank" rel="noopener noreferrer" className="text-accent-400 hover:underline font-semibold">X.com (Twitter)</a>
                    <a href={PROJECT_LINKS.telegramGroup} target="_blank" rel="noopener noreferrer" className="text-accent-400 hover:underline font-semibold">Telegram Group</a>
                    <a href={PROJECT_LINKS.discord} target="_blank" rel="noopener noreferrer" className="text-accent-400 hover:underline font-semibold">Discord</a>
                </div>
            </Section>
        </div>
    );
