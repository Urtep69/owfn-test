...
export const CaseCard = ({ socialCase }: { socialCase: SocialCase }) => {
    ...
    return (
        <div className="bg-primary-800 rounded-lg shadow-3d overflow-hidden flex flex-col">
            <img src={socialCase.imageUrl} alt={title} className="w-full h-48 object-cover" />
            <div className="p-6 flex-grow flex flex-col">
                <span className="text-sm font-semibold text-accent-500 mb-1">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                <h3 className="text-xl font-bold mb-2 h-14">{title}</h3>
                <p className="text-primary-400 mb-4 flex-grow">{description.substring(0, 100)}...</p>
                ...
                <Link to={`/impact/case/${socialCase.id}`} className="mt-auto block w-full text-center bg-accent-500 text-primary-950 font-bold py-2 rounded-lg hover:bg-accent-600 transition-colors">
                    {t('view_case')}
                </Link>
            </div>
        </div>
