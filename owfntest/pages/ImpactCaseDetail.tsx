...
export default function ImpactCaseDetail() {
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <Link to={`/impact/category/${categorySlug}`} className="inline-flex items-center gap-2 text-accent-400 hover:underline">
                <ArrowLeft size={16} /> {t('back_to_category_cases', { category: categoryName })}
            </Link>
            <div className="bg-primary-800 rounded-lg shadow-3d-lg overflow-hidden">
                <img src={socialCase.imageUrl} alt={title} className="w-full h-64 md:h-96 object-cover" />
                <div className="p-6 md:p-10">
                    <span className="text-lg font-semibold text-accent-500 mb-2 inline-block">{t(`category_${socialCase.category.toLowerCase().replace(' ', '_')}`, { defaultValue: socialCase.category })}</span>
                    <h1 className="text-3xl md:text-5xl font-bold mb-6">{title}</h1>
                    <p className="text-lg text-primary-300 leading-relaxed mb-8">{description}</p>
                    
                    <div className="mb-8">
                        <ProgressBar progress={progress} />
                        <div className="flex justify-between text-lg font-semibold mt-2">
                            <span><span className="font-bold text-accent-400">${socialCase.donated.toLocaleString()}</span> {t('funded')}</span>
                            <span><span className="font-normal text-primary-400">{t('goal')}:</span> ${socialCase.goal.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-3 space-y-8">
                     <div className="bg-primary-800 p-6 rounded-lg shadow-3d">
                        ...
                        <div className="space-y-4">
                            {mockUpdates.map(update => (
                                <div key={update.date} className="border-b border-primary-700 pb-4 last:border-b-0 last:pb-0">
                                    <p className="text-sm text-primary-400 font-semibold">{update.date}</p>
                                    <p className="text-primary-300 mt-1">{t(update.key)}</p>
                     ...
                     {details && (
                        <div className="bg-primary-800 p-6 rounded-lg shadow-3d">
                            ...
                            <p className="text-primary-300 leading-relaxed whitespace-pre-wrap">{details}</p>
                        </div>
                    )}
                </div>

                <div className="lg:col-span-2 space-y-8">
                    <div className="bg-primary-800 p-6 rounded-lg shadow-3d lg:sticky top-24">
                        <h3 className="text-2xl font-bold mb-4 flex items-center gap-2">
                            <Heart className="text-red-500" />
                            <span>{t('support_this_cause')}</span>
                        </h3>
                        <div className="bg-accent-900/20 border border-accent-500/30 text-accent-200 p-3 rounded-md text-sm flex items-start space-x-2 mb-6">
                    ...
                    <div className="bg-primary-800 p-6 rounded-lg shadow-3d">
                        <h3 className="text-2xl font-bold mb-4 flex items-center gap-3"><Milestone /> {t('funding_milestones')}</h3>
                         <div className="relative pl-4">
                            <div className="absolute top-0 left-4 h-full w-0.5 bg-primary-700"></div>
                            ...
                            {milestones.map(milestone => (
                                <div key={milestone.percentage} className="flex items-start mb-4 relative">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center -ml-8 flex-shrink-0 z-10 ${progress >= milestone.percentage ? 'bg-accent-500' : 'bg-primary-600'}`}>
                                        <CheckCircle size={16} className="text-white"/>
                                    </div>
                                    <div className="ml-4">
                                        <p className="font-bold">{milestone.percentage}%</p>
                                        <p className="text-sm text-primary-400">{t(milestone.key)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
