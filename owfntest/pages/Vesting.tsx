...
const ConnectWalletPrompt = () => {
    ...
    return (
        <div className="text-center p-12 bg-primary-800 rounded-lg shadow-3d">
            ...
            <p className="text-primary-400 mb-6">{t('vesting_connect_prompt')}</p>
            <button
...
const NoScheduleInfo = () => {
    ...
    return (
        <div className="text-center p-12 bg-primary-800 rounded-lg shadow-3d">
            <Calendar className="mx-auto w-16 h-16 text-accent-500 mb-4" />
            <h2 className="text-2xl font-bold mb-2">{t('no_vesting_schedule')}</h2>
            <p className="text-primary-400">{t('no_vesting_schedule_desc')}</p>
        </div>
    );
}
...
const VestingScheduleDetails = ({ schedule }: { schedule: VestingSchedule }) => {
    ...
    return (
        <div className="bg-primary-800 p-8 rounded-lg shadow-3d space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-x-8 gap-y-4 border-t border-primary-700 pt-6">
            ...
            <div className="bg-accent-900/30 p-6 rounded-lg text-center">
                <p className="text-lg font-semibold text-accent-300">{t('claimable_now')}</p>
                <p className="text-4xl font-bold my-2 text-accent-400">{claimableNow.toLocaleString(undefined, {maximumFractionDigits: 2})}</p>
                <button onClick={handleClaim} disabled={solana.loading || claimableNow <= 0} className="mt-4 w-full max-w-xs bg-accent-500 text-primary-950 font-bold py-3 rounded-lg text-lg hover:bg-accent-600 transition-colors disabled:opacity-50">
    ...
    return (
        <div className="mt-12 space-y-8">
            <div className="bg-primary-800 p-8 rounded-lg shadow-3d">
                <h2 className="text-2xl font-bold mb-6 flex items-center"><PlusCircle className="mr-3 text-accent-400"/>{t('create_vesting_schedule')}</h2>
                <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                    <input type="text" placeholder={t('recipient_address')} value={recipient} onChange={e => setRecipient(e.target.value)} required className="w-full p-3 bg-primary-700 rounded-md" />
                    <input type="number" placeholder={t('total_amount_owfn')} value={amount} onChange={e => setAmount(e.target.value)} required className="w-full p-3 bg-primary-700 rounded-md" />
                    <input type="number" placeholder={t('vesting_duration_months')} value={duration} onChange={e => setDuration(e.target.value)} required className="w-full p-3 bg-primary-700 rounded-md" />
                    <input type="number" placeholder={t('cliff_period_months')} value={cliff} onChange={e => setCliff(e.target.value)} required className="w-full p-3 bg-primary-700 rounded-md" />
                    <button type="submit" className="md:col-span-2 w-full bg-accent-500 text-primary-950 py-3 rounded-lg font-bold hover:bg-accent-600 transition-colors">{t('create_schedule')}</button>
                </form>
            </div>
            <div className="bg-primary-800 p-8 rounded-lg shadow-3d">
                 ...
                 <div className="space-y-4">
                     {vestingSchedules.map(schedule => (
                         <div key={schedule.recipientAddress} className="bg-primary-700/50 p-4 rounded-lg">
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            <div className="text-center">
                <h1 className="text-4xl font-bold text-accent-400">{t('vesting_title')}</h1>
